...
class RmqChannel(private val rmqDetails: RmqDetails) : Closeable {
    ...
    /**
     * Listens for messages from the RMQ server on a specific queue and invokes the specified callback function for each received message.
     *
    ...
    fun consume(acknowledgeResponse: Any?, callback: (message: Any) -> Unit): String? {
        ...
        return channel!!.basicConsume(rmqDetails.queue.name, acknowledgeResponse == null, object : DefaultConsumer(channel) {
