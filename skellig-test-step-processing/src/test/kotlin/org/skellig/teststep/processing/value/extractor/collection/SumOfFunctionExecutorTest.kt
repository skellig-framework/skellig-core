...
class SumOfFunctionExecutorTest {
    ...
    fun testSumExpression() {
        assertEquals(BigDecimal(4), functionExecutor.extractFrom("", listOf(mapOf(Pair("price", 1)), mapOf(Pair("price", 3))), arrayOf({ v: Any? -> (v as Map<*, *>)["price"] })))
        assertEquals(
            BigDecimal("5.095"), functionExecutor.extractFrom(
                "sumOf", listOf(mapOf(Pair("price", 3.09)), mapOf(Pair("price", 1)), mapOf(Pair("price", 1.005))), arrayOf({ v: Any? -> (v as Map<*, *>)["price"] })
            )
        )
        assertEquals(
            BigDecimal("35.0"), functionExecutor.extractFrom(
                "sumOf",
                listOf(
        ...
        assertEquals(
            BigDecimal("6"), functionExecutor.extractFrom(
                "sumOf",
                listOf("1", "2", "3"),
                arrayOf({ v: Any? -> v })
            )
