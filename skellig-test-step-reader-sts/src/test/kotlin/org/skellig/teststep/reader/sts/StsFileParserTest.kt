...
class StsFileParserTest {
    ...
    fun testParseSimpleTestStep() {
        ...
        Assertions.assertAll(
                ...
                { Assertions.assertEquals("\${password}", getValueFromMap(secondTestStep, "auth", "password")) },
                {
    ...
    fun testParseTestStepWithComplexValidation() {
        ...
        val firstTestStep = testSteps[0]
        Assertions.assertAll(
                { Assertions.assertEquals("T 1 2 3", getValueFromMap(firstTestStep, "validate", "from test")) },
                { Assertions.assertEquals("application/json", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "headers", "content-type")) },  // spaced inside the value must be preserved
                { Assertions.assertEquals("contains(fail  1 )", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "log", "none_match", 0)) },
                { Assertions.assertEquals("contains( error)", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "log", "none_match", 1)) },
                { Assertions.assertEquals("v3", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "body", "regex(.*f3=(\\\\w+).*)")) },
                { Assertions.assertEquals("v2", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "body", "json_path(f1.f3)")) },
                { Assertions.assertEquals("v1", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "body", "json_path(f1.f2)")) },
                { Assertions.assertEquals("200", getValueFromMap(firstTestStep, "validate", "any match", "[srv1, srv2, srv3]", "status")) }
        )
    }
